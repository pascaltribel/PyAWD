...
class VectorAcousticWaveDataset3D(VectorAcousticWaveDataset):
    ...
    def __init__(self, size: int, dx: float = 1000/32., nx: int = 32, sx: float = 1., ddt: float = 0.01,
                 ...
                 generate_data: bool = True, openmp: bool = False):
        """
        Args:
            ...
            generate_data (bool): Whether generating the data (or not to load it)
        ...
        super().__init__(size=size, dx=dx, nx=nx, sx=sx, ddt=ddt, dt=dt, t=t, interrogators=interrogators,
                         attenuation_factor=attenuation_factor, keep_full_data=keep_full_data, openmp=openmp)
    ...
    def generate_data(self):
        ...
        for idx in tqdm(range(self.size)):
            ...
            for interrogator in self.interrogators:
                self.interrogators_data[interrogator].append(torch.Tensor(data[:, :, interrogator[0] +
...
if __name__ == "__main__":
    d = VectorAcousticWaveDataset3D(10, interrogators=[(0, -8, 15), (0, 8, 15)], keep_full_data=False)